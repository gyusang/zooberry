...
let print_intr = function
  | Intr.Bot -> print_string "intr_bot"
  | Intr.Enabled -> print_string "enabled"
  | Intr.Disabled -> print_string "disabled"
  | Intr.Top -> print_string "intr_top"

let print_val ((((itv, pow_loc), array_blk), pow_proc), intr) =
  open_box 1;
  print_string "(";
  print_itv itv;
  print_string ",";
