...
Require Import vgtac VocabA Syn DLat DUnit DStr DSum DMap DProd DPow
        DItv UserInputType.
Require DMem.
Require Import DomBasic DomArrayBlk DomIntr.

(** ** Value domain *)

Module Val <: LAT := Prod5 Itv PowLoc ArrayBlk PowProc Intr.
...
Definition intr_of_val : Val.t -> Intr.t := Val.fifth.

Lemma intr_of_val_mor : Proper (Val.eq ==> Intr.eq) intr_of_val.
Proof. unfold intr_of_val; intros v1 v2 Hv. by apply Val.fifth_mor. Qed.

Definition val_of_itv (x : Itv.t) : Val.t :=
  (x, PowLoc.bot, ArrayBlk.bot, PowProc.bot, Intr.bot).

Lemma val_of_itv_mor : Proper (Itv.eq ==> Val.eq) val_of_itv.
Proof.
intros i1 i2 Hi.
split; [split; [split; [split|]|]|]; s; try assumption
; try apply Itv.eq_refl; try apply PowLoc.eq_refl; try apply ArrayBlk.eq_refl
; try apply PowProc.eq_refl; try apply Intr.eq_refl.
Qed.

Definition val_of_pow_loc (x : PowLoc.t) : Val.t :=
  (Itv.bot, x, ArrayBlk.bot, PowProc.bot, Intr.bot).
...
Definition val_of_array (x : ArrayBlk.t) : Val.t :=
  (Itv.bot, PowLoc.bot, x, PowProc.bot, Intr.bot).
...
Definition val_of_pow_proc (x : PowProc.t) : Val.t :=
  (Itv.bot, PowLoc.bot, ArrayBlk.bot, x, Intr.bot).
...
Definition val_of_intr (x : Intr.t) : Val.t :=
  (Itv.bot, PowLoc.bot, ArrayBlk.bot, PowProc.bot, x).

Lemma val_of_intr_mor : Proper (Intr.eq ==> Val.eq) val_of_intr.
Proof.
intros i1 i2 Hi.
split; [split; [split; [split|]|]|]; s; try assumption
; try apply Itv.eq_refl; try apply PowLoc.eq_refl; try apply ArrayBlk.eq_refl
...
Definition modify_array (x : Val.t) (a : ArrayBlk.t) : Val.t :=
  (itv_of_val x, pow_loc_of_val x, a, pow_proc_of_val x, intr_of_val x).
...
Definition modify_pow_proc (x : Val.t) (p : PowProc.t) : Val.t :=
  (itv_of_val x, pow_loc_of_val x, array_of_val x, p, intr_of_val x).

Lemma modify_pow_proc_mor : Proper (Val.eq ==> PowProc.eq ==> Val.eq) modify_pow_proc.
Proof.
unfold modify_pow_proc.
intros [[[[x1 y1] z1] a1] b1] [[[[x2 y2] z2] a2] b2]
       [[[[H1 H2] H3] H4] H5] i1 i2 Hi.
split; [split; [split; [split|]|]|]; assumption.
Qed.

Definition modify_intr (x : Val.t) (intr : Intr.t) : Val.t :=
  (itv_of_val x, pow_loc_of_val x, array_of_val x, pow_proc_of_val x, intr).

Lemma modify_intr_mor : Proper (Val.eq ==> Intr.eq ==> Val.eq) modify_intr.
Proof.
unfold modify_intr.
intros [[[[x1 y1] z1] a1] b1] [[[[x2 y2] z2] a2] b2]
       [[[[H1 H2] H3] H4] H5] i1 i2 Hi.
